...
Example: <?php RSSImport(10, "https://www.fwpaec.org/Online/searchExtract?search_from=4%2F1%2F18%2C%2012%3A00%20AM&search_to=8%2F21%2F18%2C%2012%3A00%20AM&organization_short_description_filter=&venue_filter=&city_filter=&search_criteria=&category_filter=&keywords=&object_type_filter=&filter1=&filter2=&filter3=&filter4=&multifilter1=&multifilter2=&filter_parent1=&filter_child1=&filter_parent2=&filter_child2=&templates=2E251B11-8BE6-484C-8D03-72ACE3980FAF"); ?>
------------------------------------------------------------
*/
...
function RSSImport(
	$display = 5,
	$feedurl = 'https://www.fwpaec.org/Online/searchExtract?search_from=4%2F1%2F18%2C%2012%3A00%20AM&search_to=8%2F21%2F18%2C%2012%3A00%20AM&organization_short_description_filter=&venue_filter=&city_filter=&search_criteria=&category_filter=&keywords=&object_type_filter=&filter1=&filter2=&filter3=&filter4=&multifilter1=&multifilter2=&filter_parent1=&filter_child1=&filter_parent2=&filter_child2=&templates=2E251B11-8BE6-484C-8D03-72ACE3980FAF',
	$before_desc = '',
	...
	$creator = 0,
	$after_creator = '</small>',
	$start_items = '<ul id="rss_ul" " style="list-style-type: none">',
	$end_items = '</ul>',
	$start_item = ' <li id="rss_li">',
	$end_item = '</li>',
	$target = '',
	$rel = '',
	$desc4title = 0,
	...
	if ( $rss && ! is_wp_error( $rss ) ) {
		...
		while ( $display < $displaylimit ) {
			...
			if ( array_key_exists( $i, $items ) ) {
				...
				$description = '';
				if ( $use_simplepie ) {
					$description = esc_attr( strip_tags( $item->get_description() ) );
				} elseif ( isset( $item[ 'title' ] ) ) {
					$title = esc_attr( strip_tags( $item[ 'title' ] ) );
				}
				// import on sale date
				$on_sale_date = '';
				if ( $use_simplepie ) {
					$on_sale_date =  $item->get_item_tags('http://audienceview.com/schema/rss', 'on_sale_date');
				} elseif ( isset( $item[ 'date' ] ) ) {
					$on_sale_date = esc_attr( strip_tags( $item[ 'on_sale_date' ] ) );
				}
				// on sale date to string, then to timestamp, then to final display format
				$on_sale_date_str = $on_sale_date[0]["data"];

				$timestamp_on_sale_date = strtotime($on_sale_date_str);

				// import event start date
				$start_date = '';
				if ( $use_simplepie ) {
					$start_date =  $item->get_item_tags('http://audienceview.com/schema/rss', 'start_date');
				} elseif ( isset( $item[ 'date' ] ) ) {
					$start_date = esc_attr( strip_tags( $item[ 'start_date' ] ) );
				}
				// date to string, then to timestamp, then to final display format
				$start_date_str = $start_date[0]["data"];

				$timestamp = strtotime($start_date_str);

				$formatted_date = date('l F jS\, \a\t g:i A', $timestamp);
			
				// import image
				$image = '';
				if ( $use_simplepie ) {
					$image = $item->get_item_tags('http://audienceview.com/schema/rss', 'image1');
				} elseif ( isset( $item[ 'date' ] ) ) {
					$image = esc_attr( strip_tags( $item[ 'image1' ] ) );
				}
				// import info_link
				$info_link = '';
				if ( $use_simplepie ) {
					$info_link = $item->get_item_tags('http://audienceview.com/schema/rss', 'additional_info');
				} elseif ( isset( $item[ 'date' ] ) ) {
					$image = esc_attr( strip_tags( $item[ 'image1' ] ) );
				}
				// import picture_url
				...
				// AV items
				$start_item_temp  = str_replace( '%start_date%', $start_date, $start_item_temp );
				// AV end
				//$echo            = 'abc'
...
/* 				// import date
				...
				//render in html
				$echo .= '<div class="RSS_left">';
					$echo .= '<a' . $target . $rel . ' href="' . $info_link[0]["data"] . '" title="' . $atitle . '">' . '<img src="' . $image[0]["data"] . '">' . '</a>';
					$echo .= '<br>';
				$echo .= '</div>';
				$echo .= '<div class="RSS_main">';
					$echo .= '<a id="RSS_title"' . $target . $rel . ' href="' . $info_link[0]["data"] . '" title="' . $atitle . '">' . $title . '</a>';
					$echo .= '<br>';
					$echo .= '<span id="RSS_date">' . $formatted_date . '</span>';
					$echo .= '<br>';
					$echo .= '<span id="RSS_desc">' . substr($description, 0, 125) . '...</span>'; 
					$echo .= '<br>';
				$echo .= '</div>';
				$echo .=  '<div class="RSS_right">';  
					if (empty($timestamp_on_sale_date)) {
						$echo .=  '<a class="btn btn-buy" href="' . $info_link[0]["data"] . '" title="' . $atitle . '"><span>Buy</span></a>';
					}
					else {
						$echo .= '';
					}
					...
					// $echo .=  date('l F jS\, \a\t g:i A', $timestamp_on_sale_date);
				$echo .= '</div>';
	...
	<script type="text/javascript" charset="utf-8">
		...
		var id = 'rssimport',
			...
			start = '[RSSImport display="5" feedurl="http://feedurl.com/" before_desc="<br />" displaydescriptions="TRUE" after_desc=" " ' +
				...
				'after_creator="</small>" start_items="<ul id="rss_ul" ">" end_items="</ul>" start_item=" <li id="rss_li">" end_item="</li>" target="" rel="" ' +
...
function RSSImport_insert_button_old() {
	...
	<script type="text/javascript">
		...
		if ( typeof edButtons !== \'undefined\' ) {
	     ...
	     ' date_format="" before_creator=" <small>" creator="FALSE" after_creator="</small>" start_items="<ul id="rss_ul" ">" end_items="</ul>"' .
	     ' start_item=" <li id="rss_li">" end_item="</li>" target="" rel="" desc4title="" charsetscan="FALSE" debug="FALSE" before_noitems="<p>"' .
...
if ( class_exists( 'WP_Widget' ) ) {
	...
	class RSSImport_Widget extends WP_Widget {
		...
		public function widget( $args, $instance ) {
			...
			$feedurl             = empty( $instance[ 'feedurl' ] ) ? 'https://www.fwpaec.org/Online/searchExtract?search_from=4%2F1%2F18%2C%2012%3A00%20AM&search_to=8%2F21%2F18%2C%2012%3A00%20AM&organization_short_description_filter=&venue_filter=&city_filter=&search_criteria=&category_filter=&keywords=&object_type_filter=&filter1=&filter2=&filter3=&filter4=&multifilter1=&multifilter2=&filter_parent1=&filter_child1=&filter_parent2=&filter_child2=&templates=2E251B11-8BE6-484C-8D03-72ACE3980FAF' : $instance[ 'feedurl' ];
			...
			$start_items         = empty( $instance[ 'start_items' ] ) ? '<ul id="rss_ul" " style="list-style-type: none">' : $instance[ 'start_items' ];
			$end_items           = empty( $instance[ 'end_items' ] ) ? '</ul>' : $instance[ 'end_items' ];
			$start_item          = empty( $instance[ 'start_item' ] ) ? ' <li id="rss_li">' : $instance[ 'start_item' ];
